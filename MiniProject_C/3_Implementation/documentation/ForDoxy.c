/** @file ForDoxy.c 
 * @brief header file
 * @bug no bug found
 * @author Nidhi Chawla
*/

#include<stdio.h>
#include<ctype.h>
#include<stdlib.h>
#include<string.h>

void display_records();
void reset_records();
void help();
void edit_records(float , char []);

/** @brief Entrypoint.
 * Main function marks the entry / start of the quiz program which displays the main menu of Covid Quiz.
 * You will use this to play the quiz and to explore the different outcomes. Rightnow it is displaying the homescreen.
 * @param   no parameters are given
 * 
 * @return should not return anything.
 * 
 * 
 */

int main()
     {
		 /** @brief Variable declaration of int,float and char */
     int countr,r,r1,count,i,n;
     float score;
     char choice;
	 char input;
     char candidate[20];
     system("cls||clear");
/** @brief **mainhomescreen**
 * Label mainhomescreen designates the options which would be given to the user / candidate when he/she starts playing the quiz to win some 
amazing gift vouchers.
* You will be giving option to start the game, view the highest records , reset the records along with Help instructions. 
*/
mainhomescreen:
     printf("\n\t\t\t\t\tCOVID QUIZ MASTER \n");
     printf("\n\t\t\t\t\t_________________________________________________________________");
     printf("\n\t\t\t\t\tTEST YOUR KNOWLEDGE ON COVID-19 AND WIN CASH VOUCHERS!!! ");
     printf("\n\t\t\t\t\t__________________________________________________________________");
     printf("\n\t\t\t\t\t__________________________________________________________________");
     printf("\n\t\t\t\t\t > Press S -> START THE GAME ");
     printf("\n\t\t\t\t\t > Press V -> VIEW THE SCORES  ");
     printf("\n\t\t\t\t\t > Press R -> RESET SCORES ");
     printf("\n\t\t\t\t\t > press H -> HELP           ");
     printf("\n\t\t\t\t\t > press Q -> QUIT             ");
     printf("\n\t\t\t\t\t__________________________________________________________________\n\n");
fflush(stdin);
printf("\n\t\t\t\t\tEnter option : ");
	 scanf(" %c",&input);
     choice=toupper(input);
     if (choice=='R')
	{reset_records();
	goto mainhomescreen;}
    else if (choice=='H')
	{
	help();
	goto mainhomescreen;
	}
	else if (choice=='V')
	{
	display_records();
	goto mainhomescreen;
	}
	else if (choice=='Q')
	exit(1);
    else if(choice=='S')
    {
		printf(" ");
     system("cls||clear");
    printf("\n\n\n\n\n\n\n\n\n\n\t\t\t\tRegister your name:");
    scanf("%s",candidate);
    system("cls||clear");
    printf("\n\t\t\t\t\t------------------  Welcome %s to COVID QUIZ MASTER --------------------------",candidate);
	printf("\n");
    printf("\n\t\t\t\t\t Here are some tips you might wanna know before playing:");
    printf("\n\t\t\t\t\t -------------------------------------------------------------------------");
    printf("\n\t\t\t\t\t >> There are 2 rounds in this Quiz Game,WARMUP ROUND & CHALLANGE ROUND");
    printf("\n\t\t\t\t\t >> In warmup round you will be asked a total of 3 questions to test your");
    printf("\n\t\t\t\t\t    general knowledge. You are eligible to play the game if you give atleast 2");
    printf("\n\t\t\t\t\t    right answers, otherwise you can't proceed further to the Challenge Round.");
    printf("\n\t\t\t\t\t >> Your game starts with CHALLANGE ROUND. In this round you will be asked a");
    printf("\n\t\t\t\t\t    total of 10 questions. Each right answer will be awarded Rs.10000!");
    printf("\n\t\t\t\t\t    By this way you can win cash Vouchers !!!!!..........");
    printf("\n\t\t\t\t\t >> You will be given 4 options and you have to press A, B ,C or D for the");
    printf("\n\t\t\t\t\t    right option.");
    printf("\n\t\t\t\t\t >> You will be asked questions continuously, till right answers are given");
    printf("\n\t\t\t\t\t >> No negative marking for wrong answers!");
    printf("\n\t\t\t\t\t!!!!!!!!!!!!! ALL THE BEST !!!!!!!!!!!!!");
    printf("\n\n\n Press Y  to start the game or Press any other key to return to the main menu !\n");
    scanf(" %c",&input);
 	 printf("%c", input);
    if (toupper(input)=='Y')
		{

		    goto home;
        }
	else
		{
        goto mainhomescreen;
      system("cls||clear");
      }
	/** @brief **Home**
	 * Label home to start the warm up round for the candidate. 
	* There will be a total of 6 questions in which he needs to make at least 2 correct to move ahead with the challenge round. 
	  */
	 home:
     system("cls||clear");
     count=0;
     for(i=1;i<=6;i++)
     {
    system("cls||clear");
    r1=i;
    switch(r1)
		{
		case 1:
		printf("\n\nQ.How is COVID-19 passed on?");
		printf("\n\nA.Through droplets from mouth and nose\t\tB.By drinking unclean water\n\nC.By exercising\t\t\t\t\tD.All of the above\n");
		scanf(" \n%c", &input);
		if (toupper(input)=='A')
			{
			    printf("\n\nCorrect!!!");count++;
			    break;}
		else
		       {
		           printf("\n\nWrong!!! The correct answer is A.Through droplets from mouth and nose");
		       break;
		       }
        case 2:
		printf("\n\n\nQ.Common symptoms of Covid-19");
		printf("\n\nA.A new and continuous cough\t\tB.Fever\n\nC.Tiredness\t\t\t\tD.All of the above\n");
		scanf(" %c", &input);
		if (toupper(input)=='D')
			{printf("\n\nCorrect!!!");count++;
			break;}
		else
		       {printf("\n\nWrong!!! The correct answer is D.All of the above");
		       break;}
        case 3:
		printf("\n\n\nQ.Can you always tell if someone has COVID-19?");
		printf("\n\nA.No, not everyone has symptoms\t\t\t\tB.Yes, Person with COVID-19 coughs a lot\n\nC.Yes, Person with COVID-19 sneeze a lot\t\tD.None of the above\n");

		scanf(" %c", &input);
		if (toupper(input)=='A')
			{printf("\n\nCorrect!!!");count++;
			break;}
		else
		       {printf("\n\nWrong!!! The correct answer is A.No,not everyone has symptoms");
		       break;}
        case 4:
		printf("\n\n\nQ.Can washing your hands protect you from COVID-19?");
		printf("\n\nA.Yes using strong bleach\tB.Yes normal soap and water\n\nC.Yes with just water\t\tD.None of the above\n");
		scanf(" %c", &input);
		if (toupper(input)=='B')
			{printf("\n\nCorrect!!!");count++;
			 break;}
		else
		       {printf("\n\nWrong!!! The correct answer is B.Yes normal soap and water or hand sanitizer is enough");
		       break;}
        case 5:
        printf("\n\n\nQ.How many countries, areas or territories are suffering from novel coronavirus outbreak in the World?");
        printf("\n\nA.More than 50\t\tB.More than 100\n\nC.More than 150\t\tD.More than 200\n");
		scanf(" %c", &input);
        if (toupper(input)=='D')
              {printf("\n\nCorrect!!!");count++;
                break;}
        else
		       {printf("\n\nWrong!!! The correct answer is D.More than 200");
		       break;}
        case 6:
		printf("\n\n\nQ.In a study, which cells are found in COVID-19 patients 'bode well' for long term immunity?");
		printf("\n\nA.P-Cell\t\tB.D-Cell\n\nC.T-Cell\t\tD.None of the above\n");
		scanf(" %c", &input);
		if (toupper(input)=='C' )
			{printf("\n\nCorrect!!!");count++;
			break;}
		else
		       {printf("\n\nWrong!!! The correct answer is C.T-Cell");
		       break;}}
		       }
	/** @brief 
	 * if the user was able to clear the warm up round with at least 2 correct questions , he is re-directed to take challenge round 
	*  Else user will not be elligible to play the game and will be redirected to home screen.*/

	if(count>=2)
	{goto starttest;}
	else
	{
	system("cls||clear");
	printf("\n\t\t\t\t\tSORRY YOU ARE NOT ELIGIBLE TO PLAY THIS GAME, WOULD YOU LIKE TO TRY AGAIN ?\n");
	goto mainhomescreen;
	}
     starttest:
     system("cls||clear");
     printf("\n\n\t*** CONGRATULATION %s you are eligible to play the Game ***",candidate);
     printf("\n\n\n\n\t!Press'P' to Start the Game!");
	 scanf(" %c", &input);
     if(toupper(input)=='p')
		{goto game;}
/** @brief **Game Function**
 * game label is the actual money game that user has to clear some amazing vouchers. 
 * Depending on the right answers the candidate will be given cash prize.
 * @param[in] i
 * @param[out] countr
 * countr isinitialized to 0, value increses with each correct answer.
 * @return returns countr which displays the no of correct answers given by the user. 
 */
game:
     countr=0;
     for(i=1;i<=10;i++)
     {

         system("cls||clear");
     r=i;
     switch(r)
		{
		case 1:
		printf("\n\n\nQ1.Name the vaccine that is jointly developed by the German company BioNTech and US pharma giant Pfizer for COVID-19?");
		printf("\n\nA.BNT162\t\tB.PICOVACC\n\nC.Both A and B\t\tD.None of the above\n");
		scanf(" %c", &input);
		if (toupper(input)=='A')
			{printf("\n\nCorrect!!!");countr++;  
			 break;  
			 }
		else
		       {printf("\n\nWrong!!! The correct answer is A.BNT162");  
		       break;}
		case 2:
		printf("\n\n\nQ2.Clinical trial in which blood is transfused from recovered COVID-19 patients to a coronavirus patient who is in\n   critical condition?");
		printf("\n\nA.Plasma Therapy\t\tB.Solidarity\n\nC.Remidisvir\t\t\tD.None of the above\n");
		scanf(" %c", &input);
		if (toupper(input)=='A')
			{printf("\n\nCorrect!!!");countr++;  
			 break;}
		else
		       {printf("\n\nWrong!!! The correct answer is A.Plasma Therapy");  
		   		       break;
		       }
        case 3:
		printf("\n\n\nQ3.The first case of novel coronavirus was identified in .....");
		printf("\n\nA.Beijing\t\tB.Shanghai\n\nC.Wuhan\t\t\tD.None of the above\n");
		scanf(" %c", &input);
		if (toupper(input)=='C')
			{printf("\n\nCorrect!!!");countr++;  
			 break;}
		else
		       {printf("\n\nWrong!!! The correct answer is C.Wuhan");  
		       break;}
        case 4:
		printf("\n\n\nQ4.Which of the following diseases are related to coronavirus?");
		printf("\n\nA.MERS\t\t\tB.SARS\n\nC.Both A and B\t\tD.None of the above\n");
		scanf(" %c", &input);
		if (toupper(input)=='B')
			{printf("\n\nCorrect!!!");countr++;  
			 break;}
		else
		       {
                printf("\n\nWrong!!! The correct answer is B.SARS");  
		       break;
		       }
        case 5:
		printf("\n\n\nQ5.How long does the novel coronavirus survive outside the body?");
		printf("\n\nA.2 Weeks\t\t\tB.A Month\n\nC.Several hours to days\t\tD.None of the above \n");
		scanf(" %c", &input);
		if (toupper(input)=='C')
			{printf("\n\nCorrect!!!");countr++;   
			break;}
		else
		       {
		           printf("\n\nWrong!!! The correct answer is C.Several hours to days");
		       break;
		       }
		case 6:
		printf("\n\n\nQ6.What is a safe distance to stay apart from someone who is sick?");
		printf("\n\nA.1 Foot\t\t\tB.3 Feet\n\nC.Both of the above\t\tD.None of the above\n");
		scanf(" %c", &input);
		if (toupper(input)=='B' )
			{printf("\n\nCorrect!!!");countr++;  
			 break;}
		else
		       {printf("\n\nWrong!!! The correct answer is B.3 Feet");
		       break;}
        case 7:
		printf("\n\n\nQ7.Which is an inactivated vaccine that it is made up of killed coronaviruses, making it safe to be injected into the body?");
		printf("\n\nA.Pfizer\t\t\tB.Moderna\n\nC.Sputnik V\t\t\tD.Covaxin\n");
		scanf(" %c", &input);
		if (toupper(input)=='D')
			{printf("\n\nCorrect!!!");countr++;  
			 break;}
		else
		       {printf("\n\nWrong!!! The correct answer is D.Covaxin");  
		       break;}
        case 8:
		printf("\n\n\nQ8.Who is at highest risk of developing severe Covid-19 disease?");
		printf("\n\nA.Children\t\t\tB.Adults\n\nC.People above 60\t\tD.None of the above\n");
		scanf(" %c", &input);
		if (toupper(input)=='C')
			{printf("\n\nCorrect!!!");countr++;   
			break;}
		else
		       {printf("\n\nWrong!!! The correct answer is C.People above 60");  
		       break;}
        case 9:
		printf("\n\n\nQ9.A Mask on your face should cover which of the following?");
		printf("\n\nA.Nose,mouth and chin\t\t\t\tB.Mouth Only is sufficient\n\nC.Mouth and Chin should be covered\t\tD.Mask is not mandatory\n");
		scanf(" %c", &input);
		if (toupper(input)=='A')
			{printf("\n\nCorrect!!!");countr++;   
			break;}
		else
		       {printf("\n\nWrong!!! The correct answer is A.Nose,mouth and chin");  
		       break;}
        case 10:
		printf("\n\n\nQ10.The distinguishing feature of a coronavirus is its:");
		printf("\n\nA.Shape\t\t\tB.Size\n\nC.Mobility\t\tD.Deadliness\n");
		scanf(" %c", &input);
		if (toupper(input)=='A')
			{printf("\n\nCorrect!!!");countr++;   
			break;}
		else
		       {printf("\n\nWrong!!! The correct answer is A.Shape");  
			   break;}}}
			   /** @brief **Score Function**
				* @param[in] countr
				* @param[out] score
				* score= (float)countr*1000
				* @return score of the user
				*/
	score:
    system("cls||clear");
	score=(float)countr*1000;
	if(score>0.00 && score<10000)
	{
	   printf("\n\n\t\t**************** CONGRATULATION *****************");
	     printf("\n\t You won Rs.%.2f",score);goto go;}
	 else if(score==10000.00)
	{
	    printf("\n\n\n \t\t**************** CONGRATULATION ****************");
	    printf("\n\t\t\t\t YOU HAVE WON THE GAME AND CASHBACK VOICHERS TOO!!!!!!!!!");
	    printf("\n\t\t You will get Rs. %.2f",score);
	    printf("\t\t Thank You!!");
	}
	 else
{
	 printf("\n\n\t******** SORRY YOU DIDN'T WIN ANY CASH ********");
	    printf("\n\t\t Thanks for your participation");
	    printf("\n\t\t TRY AGAIN");goto go;}
	/** @brief **Go**
	 * takes input from user whether to play the game again or go the main menu.
	 */
	go:
	puts("\n\n Press Y if you want to play next game");
	puts(" Press any key if you want to go main menu\n");
	scanf(" %c", &input);
	printf("%c",input);
	if (toupper(input)=='Y')
		goto home;
	else
		{
		edit_records(score,candidate);
		goto mainhomescreen;}
}
return 0;
}
/** @brief functions: void display_record()
 * function that is used to display the highest record of the candidate by reading the file in which the scores are stored. 
 * @param[in] float sc, char nm[20],c,FILE*f
 * prints the highest score
 * */
void display_records()
    {
        system("cls||clear");
    float sc;
	char nm[20],c;
	FILE *f;
	f=fopen("score.txt","r");
	fscanf(f,"%s%f",nm,&sc);
	fprintf(stdout,"\n\t\t\t\t\t*************************************************************");
	printf("\n\t\t\t\t\t %s has secured the Highest Score %0.2f",nm,sc);
	printf("\n\t\t\t\t\t*************************************************************");
	fclose(f);
	printf("\n");
	}
/** @brief function: void reset_records()
 * function used to reset the records of the candidate who have played the game , so that you can go ahead and play a new game to achieve more
 * @param[in]  float sc,char nm[20],FILE*f 
 * prints: scores have been reset, go ahead and set a new score now!
 */
void reset_records()
    {
        system("cls||clear");
    float sc;
	char nm[20];
	FILE *f;
	f=fopen("score.txt","r+");
	fscanf(f,"%s%f",nm,&sc);
	f=fopen("score.txt","w+");
	fclose(f);
	sc=0;
	f=fopen("score.txt","w+");
	fprintf(f,"%s",nm);
	printf("\n\t\t\t\t\tScores have been reset, Go ahead and set a new score now!!!");
    fclose(f);
	printf("\n");}
	/** @brief function: void help()
	 * this function helps the user to get the overview of the game and makes the user aware of all the rules and regulations.
	 * @param no parameters are given
	 * @return nothing is returned
	 */
void help()
	{
	    system("cls||clear");
    printf("\n\t\t\t\t\t                              HELP");
    printf("\n\t\t\t\t\t -------------------------------------------------------------------------");
    printf("\n\t\t\t\t\t >> There are two rounds in the game, WARMUP ROUND & CHALLANGE ROUND");
    printf("\n\t\t\t\t\t >> In warmup round you will be asked a total of 3 questions to test your general");
    printf("\n\t\t\t\t\t    knowledge. You will be eligible to play the game if you can give atleast 2");
    printf("\n\t\t\t\t\t    right answers otherwise you can't play the Game...........");
    printf("\n\t\t\t\t\t >> Your game starts with the CHALLANGE ROUND. In this round you will be asked");
    printf("\n\t\t\t\t\t    total 10 questions each right answer will be awarded Rs. 10000.");
    printf("\n\t\t\t\t\t    By this way you can win cash prize in INR...............");
    printf("\n\t\t\t\t\t >> You will be given 4 options and you have to press A, B ,C or D for the");
    printf("\n\t\t\t\t\t    right option");
    printf("\n\t\t\t\t\t >> You will be asked questions continuously if you keep giving the right answers.");
    printf("\n\t\t\t\t\t >> No negative marking for wrong answers");
	printf("\n\t\t\t\t\t*********************BEST OF LUCK*********************************");
	printf("\n");
	printf("\n");
	printf("\n\t\t\t\t\t*********************Press any Option mention below to start the Game.*********************************");
	}

/** @brief function: void edit_records()
 * function used to edit the records of the candidate , this is being called every time the user gives a correct answer to update the record in the file. 
 * @param[in] float sc, char nm[20],FILE*f
 * if the score is greater than the previous recorded score then the score gets updated.
 * @return nothing is returned
 */
void edit_records(float score, char plnm[20])
	{
	    system("cls||clear");
	float sc;
	char nm[20];
	FILE *f;
	f=fopen("score.txt","r");
	fscanf(f,"%s %f",nm,&sc);
	if (score>=sc)
	  { sc=score;
	    fclose(f);
	    f=fopen("score.txt","w");
	    fprintf(f,"%s\n%.2f",plnm,sc);
	    fclose(f);
}
}



